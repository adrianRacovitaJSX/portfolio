...
export function PlaceholdersAndVanishInput({
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter" && !animating && !isLoading) {
      vanishAndSubmit();
    }
  };
  ...
  const vanishAndSubmit = async () => {
    ...
    if (value && inputRef.current) {
      ...
      // Esperar a que termine la animación antes de hacer la llamada a la API
      await handleSubscribe(value);
    }
  };

  const handleSubscribe = async (emailValue: string) => {
    if (!emailValue || isLoading) return;
    setIsLoading(true);
      ...
      toast({
        description: (
          <div className="flex flex-col gap-1">
            ...
            <span>Te mantendré informado sobre las últimas novedades.</span>
          </div>
        ),
      });
